...
def user_1():
    user = User()
    user.updated_at = datetime(2022, 1, 27, 12, 59, 45, 596117).fromisoformat(
        "2022-01-27T12:59:45.596117+00:00"
    )
    user.is_active = True
...
def test_write_datetime_value_valid_string():
    ...
    assert "Invalid datetime string value found for property updatedAt" in str(
        excinfo.value
    )
...
def test_write_timedelta_value_invalid_string():
    with pytest.raises(ValueError) as excinfo:
        json_serialization_writer = JsonSerializationWriter()
        json_serialization_writer.write_timedelta_value(
            "diff", "invalid-timedelta-string"
        )
    assert "Invalid timedelta string value found for property diff" in str(
        excinfo.value
    )
...
def test_write_time_value():
    ...
    json_serialization_writer.write_time_value(
        "time", datetime.fromisoformat("2022-01-27T12:59:45.596117").time()
    )
    content = json_serialization_writer.get_serialized_content()
    content_string = content.decode("utf-8")
...
def test_write_collection_of_object_values(user_1, user_2):
    json_serialization_writer = JsonSerializationWriter()
    json_serialization_writer.write_collection_of_object_values(
        "users", [user_1, user_2]
    )
    content = json_serialization_writer.get_serialized_content()
    content_string = content.decode("utf-8")
    assert (
        content_string == '{"users": [{"id": "8f841f30-e6e3-439a-a812-ebd369559c36", '
...
def test_write_object_value(user_1):
    ...
    content_string = content.decode("utf-8")
    assert (
        content_string == '{"user1": {"id": "8f841f30-e6e3-439a-a812-ebd369559c36", '
        '"updated_at": "2022-01-27T12:59:45.596117+00:00", "is_active": true}}'
    )
